...
def load_data(path: str, device: torch.device, slice=None):
    ...
    item = {
        ...
        'roi': f"{path}/anat/{name}_acq-T2sel_FLAIR_roi.nii.gz"
    }
...
if __name__ == "__main__":
    ...
    net = get_network(configuration='2d', fold=0)
    net = net.to('cuda:0')

    item = load_data("nnUNet/nnUNet_raw/Dataset101_fcd/sub-00001", torch.device('cuda:0'), slice=140)
